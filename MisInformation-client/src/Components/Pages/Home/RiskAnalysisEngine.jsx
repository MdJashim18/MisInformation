 ...
 * ============================================================
 *  Usage:
 *    1. Place `riskData.json` in your public folder (e.g. /public/riskData.json)
 *    2. Import and render <RiskAnalysisEngine /> in your App
 *    3. Dependencies: React, recharts, lucide-react
...
//  Ref: Dempster (1967), Yang et al. (2006)
// ─────────────────────────────────────────────────────────────
function dsAggregate(evidences) {
  ...
  for (let e = 1; e < evidences.length; e++) {
    ...
    // Step 1: Compute conflict coefficient K
    let K = 0;
    ...
    // Step 2: Compute unnormalized combined masses
    const newM = {};
...
//  Ref: Yang et al. (2013), Wang et al. (2006)
// ─────────────────────────────────────────────────────────────
function brbAggregate(evidences, weights = null) {
  ...
  // Weighted belief degrees per rule
...
function computeAll(data) {
  ...
  for (const alt of alts) {
    // All 20 sub-criteria evidences
    const allEvs = [];
    for (const crit of data.metadata.criteria)
      for (const sub of crit.subCriteria)
        allEvs.push(sub.beliefs[alt]);

    // Per-criterion evidences
    const critResults = {};
    for (const crit of data.metadata.criteria) {
    ...
    const critDsEvs   = data.metadata.criteria.map(c =>
      critResults[c.name].ds
    );
    const critBrbEvs  = data.metadata.criteria.map(c =>
      critResults[c.name].brb
    );
              ...
              <tbody>
                {critData.subCriteria.map((sub, i) => (
                  <tr key={i} style={{ borderBottom: "1px solid #f1f5f9" }}>
                    ...
                    {CATS.map(c => (
                      <td key={c} style={{ ...TD, fontWeight: sub.raw[c] > 0.3 ? 600 : 400,
...
export default function RiskAnalysisEngine() {
  ...
  if (error) return (
    <div style={{ display: "flex", alignItems: "center", gap: 10,
      ...
      <XCircle size={18} />
      <div>
        <div style={{ fontWeight: 600 }}>Failed to load riskData.json</div>
        <div style={{ fontSize: 12, marginTop: 2 }}>{error}</div>
        <div style={{ fontSize: 12, marginTop: 4, color: "#b91c1c" }}>
          Ensure <code>riskData.json</code> is in your <code>/public</code> folder.
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* Tab bar */}
        <div style={{ display: "flex", gap: 4, marginTop: 16, borderBottom: "none" }}>
          {TABS.map(tab => {
            const Icon    = tab.icon;
          ...
          <Link to='/InputData' className="border-none flex justify-between items-center gap-1 cursor-pointer text-[#64748b] text-[13px] font-[400]"><MdOutlineInput /> Input Data</Link>
        </div>
      </div>
            ...
            <div style={{
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {[
                  ...
                  <div key={row.level} style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                    <div style={{
